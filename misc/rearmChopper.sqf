...
if !((_veh isKindOf "Helicopter" ) or (_veh isKindOf "plane")) exitWith {
_veh vehicleChat "This pad is for chopper repairs only, soldier!";
//[] call aw_serverMoveGriefer;
 };
...
_veh vehicleChat format ["Repairing and Refueling %1. Stand by...", _vehType];
...
while {_fuelLevel < 1} do
{
	...
	_veh vehicleChat format["Refueling (%1%)...", floor _percentage];
...
_veh removeMagazinesTurret ["2000Rnd_65x39_Belt_Tracer_Red",[2]];
sleep 2;

_veh vehicleChat format ["%1 successfully repaired and refueled.", _vehType];